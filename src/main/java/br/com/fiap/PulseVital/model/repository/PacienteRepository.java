...
public class PacienteRepository extends Repository {
    ...
    public boolean updatePaciente(Paciente paciente) {
        String sql = "UPDATE PACIENTE_MK1 SET nome = ?, cpf = ?, alergia = ?, tipo_sanguineo = ?, remedio_frequente = ?, " +
                "dt_nascimento = ?, sexo = ?, doencas = ?, peso = ?, tel_emergencia = ? WHERE id = ?";
        try (Connection connection = getConnection();
             PreparedStatement ps = connection.prepareStatement(sql)) {
            ps.setString(1, paciente.getNome());
            ps.setString(2, paciente.getCpf());
            ...
            ps.setString(4, paciente.getTipoSanguineo());
            ps.setString(5, paciente.getRemedioFrequente());
            ps.setDate(6, new Date(paciente.getDataNascimento().getTime())); // Convertendo para java.sql.Date
            ps.setString(7, paciente.getSexo());
            ps.setDouble(8, paciente.getPeso());
            ps.setString(9, paciente.getTelefoneEmergencia());
            // Defina outros atributos conforme sua estrutura de banco de dados
            ps.setLong(10, paciente.getId());
